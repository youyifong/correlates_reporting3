...
for (iObj in 1:2) {
  ...
  COR=paste0("Objective ",iObj," BD29 ",if(iObj==2) "fold change ","Naive")
  ...
  COR=paste0("Objective ",iObj," BD29 ",if(iObj==2) "fold change ","Non-naive")
  pathCoRoutput <- glue("cor_coxph/output/{attr(config,'config')}/obj{iObj}_nnaive")
  res = knitr::knit_child(c(here::here('cor_coxph', 'report_by_COR.Rmd')), quiet = TRUE)
  cat(res, sep = '\n')
  
  COR=paste0("Objective ",iObj," BD29 ",if(iObj==2) "fold change ","Pooled")
...
cat(glue("One model per table.\\"), sep='\n')
...
covariates_adjusted = paste0(covariates_adjusted, " + naive")
